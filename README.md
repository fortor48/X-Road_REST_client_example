...
asic_path = path/to/save/asic/files
 ...
 # Ім'я файлу сертифіката який буде згенеровано якщо не буде знайдено файл у ʼʼʼcert_pathʼʼʼ
cert_file = cert_name.pem

 # Ім'я файлу ключа який буде згенеровано якщо не буде знайдено файл у ʼʼʼcert_pathʼʼʼ
key_file = key.pem

 # Ім'я файлу сертификата Трембіти, котрий необхідний для роботи з системой Трембіта за протоколом https з взаемною аутентифікацією, має знаходитись у  ʼʼʼcert_pathʼʼʼ
