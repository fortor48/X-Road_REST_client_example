...
# Повідомлення
echo -e "${GREEN}Сертифікат та ключ (${CERT_FILE_NAME}, ${KEY_FILE_NAME}) згенеровано, у директорії ${CERTS_DIR}. Можете додати сертифікат Трембіти та запустити збірку Докер контейнеру, імʼя фалу повинно передаватись у змінній оточення TREMBITA_CERT_FILE.${RESET}"